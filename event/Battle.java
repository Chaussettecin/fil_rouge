...
//-- Incarne des événements et gère les calculs --
public class Battle {
        ...
        if (Util.isSuccess(player.getAccuracy() - (redReaperDebuff ? 40 : 0)) || buffs[Util.FOCUS]) {
            ...
            if (move.type < 2) {
                ...
                if (buffs[Util.INVERT]) {
                    ...
                    opponent.hit(damage);
                    dialog = new String[]{
                            "Tu as utilisés " + move.name + "!",
                            "il y a  " + damage + " dégats sur " + opponent.getId() + "."
                    };
                }
            }
...
//-- Réduit les dommages d'une entité par la réduction des dommages de guérison --
...
//-- Retourne les dialogues associés à chaque coup spécial --
...
// --1-3 points d'expérience supplémentaires de l'ennemi pour équilibrer la croissance des points d'expérience
    //-- * Les monstres d'élite donnent 1.5x exp et les boss donnent 3x exp
...
//-- Or gagné échelles sur le niveau ennemi et le niveau de joueur
   // Le joueur recevra moins d'or plus la différence de niveau sera grande et vice versa
...
//-- Traite les probabilités que l’élément tombe de l’ennemi et
    //-- renvoie l'article qu'ils ont déposé
     //-- Retourne la valeur null si l'ennemi ne lâche pas d'objet

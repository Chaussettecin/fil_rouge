	     ...
	     while (true) {
	          ...
	          ui.println();
	           for (int i = 0; i < joueurs_ennemis.arrayEnemies.size(); i++) {
	        	   ui.println((i + 1) + ") " + joueurs_ennemis.arrayEnemies.get(i).getNom());
	           }
	          ui.println((joueurs_ennemis.arrayEnemies.size() + 1) + ") Retour");
	          ui.println("------------------------------------------------------------");

	          int menuItem = ui.getValidInt();

	          try {
	            joueurs_ennemis.arrayEnemies.get(menuItem - 1).viewAbout();
	          } catch (Exception e) {
	              if (menuItem == (joueurs_ennemis.arrayEnemies.size() + 1)) return;

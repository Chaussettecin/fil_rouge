...
//---Classe de chargement et de stockage de la ressource principale. Utilise un AssetManager 
	//pour gérer les textures, 

//-- Contient des méthodes pratiques pour charger et obtenir des ressources du 
...
//--- Skin --
 ...
 //-- Renvoie une copie d'un élément aléatoire avec une rareté pondérée de la réserve.
    //Renvoie uniquement les éléments dont les plages contiennent un niveau donné.
    //Utilisé pour les largages de monstres basés sur le niveau de l'ennemi
    //Retourne la valeur null si aucun élément ne correspond au niveau
...
//-- Renvoie une copie d'un élément aléatoire du pool d'éléments donné la rareté
    //Ceci a été fait pour que les doublons d'acteurs d'élément puissent être créés
...
//--Renvoie une copie d'un élément indexé du pool d'éléments --
    public Item getItemFromKey(int rarity, int index) {
        Item item = items.get(rarity).get(index);
        return getItemCopy(item);
    }

//-- Renvoie un article aléatoire de la réserve avec une rare et dans une plage de niveaux donnée.--
...
//-- Renvoie un article aléatoire du groupe d’articles avec une rareté pondérée. -
...
//-- retourne un article au hasard à partir du groupe d'objets, peu importe la rareté -

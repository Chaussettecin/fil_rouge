...
import terminalOverflow.Ennemie;
import terminalOverflow.Player;
...
public class PlayScreen implements Screen {
	...
	public void displayOutput(AsciiPanel terminal) {
		...
		if (map == null) {
			...
			j = new Player(terminal,map,15,10,'@',Color.BLUE,nom);
	        createBot(terminal);
	}
			...
			for(int i=0;i<bot.size();i++) {
				((Ennemie) bot.get(i)).deplacementAlea();
				((Ennemie) bot.get(i)).afficher();
			}
			map.afficherMap();
...
//		}
	...
	public void createBot(AsciiPanel terminal) {
		...
		bot=new ArrayList();
		for(int i=0;i<9;i++) {
	    bot.add(new Ennemie(terminal, map, (Vector2d) ennemie.getEnnemiesMonde1().get(i), '#', Color.YELLOW  ));
	    }
	}
